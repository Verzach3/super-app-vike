...
function PatientCard({ patient }: { patient: CompoundPatient }) {
	return (
		<Card withBorder m={"md"}>
			<Group gap={0}>
				<Text fw={700}>EMR Id:</Text>
				<Text>{patient.emrId}</Text>
				<Text fw={700}>DB Id:</Text>
				<Text>{patient.dbId ?? "No Profile"}</Text>
			</Group>
			<Group gap={0}>
				...
				<Group>
					<Stack gap={0}>
						<Group>
							...
							<Text>{patient.getName()}</Text>
						</Group>
						<Group>
							<Text fw={700}>Nacimiento:</Text>
