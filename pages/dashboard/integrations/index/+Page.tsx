...
function Integrations() {
	...
	const [
		loadingArticles,
		{ close: closeLoadingArticles, open: openLoadingArticles },
	] = useDisclosure(false);
	...
	const nextStepArticles = () =>
		setActiveArticles((current) => (current < 3 ? current + 1 : current));
	const prevStep = () =>
		setActive((current) => (current > 0 ? current - 1 : current));
	const prevStepArticles = () =>
		setActiveArticles((current) => (current > 0 ? current - 1 : current));
	...
	async function updateBlogs() {
		...
		const res = await axios.get(
			"https://n8n.containers.wellfitclinic.com/webhook/9870ed7d-44b4-4efe-90c9-b21e8c040b20",
		);
		console.log(res.data, active);
		nextStep();
		nextStep();
		setBlogResults({
			duplicates: res.data.same ?? 0,
	...
	async function updateArticles() {
		...
		nextStepArticles();
		const res = await axios.get(
			"https://n8n.containers.wellfitclinic.com/webhook/b6166b6c-27f4-43d4-9723-745900cae429",
		);
		console.log(res.data, active);
		nextStepArticles();
		nextStepArticles();
			...
			<Stack>
				<Card withBorder>
					<Group grow justify="space-between" onClick={toggle}>
						<Text>Integracion de Blogs</Text>
						<Group justify="right">
					...
					<Collapse in={opened}>
						<Stepper active={active} mt={"2rem"}>
							<Stepper.Step
								label="Empezar Integracion"
								description="Obtener Blogs de Notion"
							...
							<Stepper.Step
								...
								description="Omitir duplicados sin cambios"
								loading={active === 2 && loading}
							>
								{loading && <Loader type="dots" />}
							</Stepper.Step>
							<Stepper.Step
								label="Subir a la Base de Datos"
								description="Actualizar en Supabase"
								loading={active === 3 && loading}
							>
								{loading && <Loader type="dots" />}
							</Stepper.Step>
							<Stepper.Completed>
				...
				<Card withBorder>
					<Group grow justify="space-between" onClick={toggleArticles}>
						<Text>Integracion de Articulos</Text>
						<Group justify="right">
							<ThemeIcon size={"lg"} variant="white" c={"black"}>
								<IconChevronDown />
							</ThemeIcon>
					...
					<Collapse in={openedArticles}>
						<Stepper active={activeArticles} mt={"2rem"}>
							<Stepper.Step
								label="Empezar Integracion"
								description="Obtener articulos de Notion"
								loading={activeArticles === 1 && loadingArticles}
							>
							...
							<Stepper.Step
								label="Detectar Duplicados"
								description="Omitir duplicados sin cambios"
								loading={activeArticles === 2 && loadingArticles}
							>
								{loadingArticles && <Loader type="dots" />}
							</Stepper.Step>
							<Stepper.Step
								label="Subir a la Base de Datos"
								description="Actualizar en Supabase"
								loading={activeArticles === 3 && loadingArticles}
							>
								{loadingArticles && <Loader type="dots" />}
							</Stepper.Step>
							<Stepper.Completed>
