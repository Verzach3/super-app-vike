...
export default function Survey() {
  ...
  return (
    ...
    >
      ...
      <ClientOnly
        load={() => import("@/components/dashboard/surveys/SurveyCreatorWidget")}
        fallback={<Center h={"100%"}>
          <Loader type="bars"/>
        </Center>}
      >
        {(SurveyCreatorWidget) => <SurveyCreatorWidget json={data?.json} id={data?.id ?? ""} onSaveSurvey={onSaveSurvey}/>}
      </ClientOnly>
    </div>
  );
}
