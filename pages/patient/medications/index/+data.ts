...
export type PatientMedicationType = Awaited<
	ReturnType<typeof getPatientMedication>
>;
...
export async function data(pageContex: PageContext) {
	...
	const patientProfile = await xata.db.patient_profiles
		.filter({ user_id: pageContex.user.uid })
		.getFirst();
	...
	return await getPatientMedication(
		pageContex.cache.data.emr_token,
		patientProfile?.emr_id,
	);
}
